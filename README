...
This is Flask-Restless-Zx, a fork of [Flask-Restless][frestless] which facilitates the creation
...
It was forked to improve certain aspects of Flask-Restless, such as the error logging and error object returns.
...
For more information, check the original flask-restless documentation. Modifications in Flask-Restless-ZX are detailed further below.

  * [Documentation](http://readthedocs.org/docs/flask-restless)
  * [Python Package Index listing](http://pypi.python.org/pypi/Flask-Restless-Zx)
  * [Source code repository](http://github.com/zxlidars/flask-restless-zx)

[1]: http://flask.pocoo.org
[sa]: http://sqlalchemy.org
[fsa]: http://packages.python.org/Flask-SQLAlchemy
[frestless]: http://github.com/jfinkels/flask-restless

## Changes in Flask-Restless-Zx ##

* Error log messages have been improved, and now use `logger.warning` instead of `logger.exception`. 
* `status` is now included in every error object, containing the status code of the error.
* Search filter error responses now have a `detail` key which provides more information regarding the error.
...
Alternatively, `pipenv` can be used to set up a virtual environment using the provided PIPFILE.
...
[11]: http://github.com/zxlidars/flask-restless-zx/issues
